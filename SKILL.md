...
Release `v0.3.1` has no automatic updater. Its executable Prompt defaults are the
...
For `v0.3.1`, use only the exact GitHub Release after verifying its separate checksum
...
`$HOME/.follow-builders/releases/0.3.1`。Codex、Claude Code 与 custom registration
...
```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; node "$FOLLOW_UP_SKILL_DIR/scripts/schedule-gate.js" --config "$HOME/.follow-builders/config.json"
```
...
```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; node "$FOLLOW_UP_SKILL_DIR/scripts/collect-and-prepare.js" --request-out <absolute-request-path> [--frequency daily|weekly] [--scheduled]
```
...
```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; node "$FOLLOW_UP_SKILL_DIR/scripts/validate-digest-selection.js" --request <absolute-request-path> --selection "$HOME/.follow-builders/state/selections/<digest-id>.json" --output "$HOME/.follow-builders/state/selections/validated/<digest-id>.json"
```

### Step 5: Finalize

```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; node "$FOLLOW_UP_SKILL_DIR/scripts/finalize-digest.js" --request <absolute-request-path> --selection "$HOME/.follow-builders/state/selections/validated/<digest-id>.json" --output-dir <absolute-output-directory>
```
...
```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; node "$FOLLOW_UP_SKILL_DIR/scripts/deliver.js" --active <absolute-output-directory>/active.json --destination <stdout|telegram|email> [--confirm-destination] --result-out <absolute-delivery-result-path>
```

自动运行必须显式标记 scheduled，使 deliver 再次执行 TOCTOU 门禁：

```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; node "$FOLLOW_UP_SKILL_DIR/scripts/deliver.js" --active <absolute-output-directory>/active.json --destination <stdout|telegram|email> --scheduled --result-out <absolute-delivery-result-path>
```

默认 stdout 也必须运行：

```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; node "$FOLLOW_UP_SKILL_DIR/scripts/deliver.js" --active <absolute-output-directory>/active.json --destination stdout --result-out <absolute-delivery-result-path>
```
...
```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; node "$FOLLOW_UP_SKILL_DIR/scripts/resolve-delivery.js" <attempt-id> delivered --result-out <absolute-resolution-result-path>
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; node "$FOLLOW_UP_SKILL_DIR/scripts/resolve-delivery.js" <attempt-id> suppress --result-out <absolute-resolution-result-path>
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; node "$FOLLOW_UP_SKILL_DIR/scripts/resolve-delivery.js" <attempt-id> retry --confirm-external-retry [--destination stdout|telegram|email] --result-out <absolute-resolution-result-path>
```
...
```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; node "$FOLLOW_UP_SKILL_DIR/scripts/deliver.js" --active <absolute-output-directory>/active.json --destination <stdout|telegram|email> --resume-attempt <replacement-attempt-id> [--confirm-destination] --result-out <absolute-delivery-result-path>
```
...
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.3.1}"; cp "$FOLLOW_UP_SKILL_DIR/prompts/<filename>.md" ~/.follow-builders/prompts/<filename>.md
```
