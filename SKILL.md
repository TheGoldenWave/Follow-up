...
Release `v0.4.0-beta.4` has no automatic updater. Its executable Prompt defaults are the
...
For `v0.4.0-beta.4`, use only the exact GitHub prerelease after verifying its separate checksum
...
`$HOME/.follow-builders/releases/0.4.0-beta.4`。Codex、Claude Code 与 custom registration
...
```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; node "$FOLLOW_UP_SKILL_DIR/scripts/schedule-gate.js" --config "$HOME/.follow-builders/config.json"
```
...
```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; node "$FOLLOW_UP_SKILL_DIR/scripts/collect-and-prepare.js" --request-out <absolute-request-path> [--frequency daily|weekly] [--scheduled]
```
...
写入后先通过确定性 validator，将验证后的 manifest 放入独立的 `validated/` 目录，
保留相同的 `<digest-id>.json` basename。原始与验证后的文件路径分别为
`~/.follow-builders/state/selections/<digest-id>.json` 和
`~/.follow-builders/state/selections/validated/<digest-id>.json`；执行时使用展开后的绝对路径。
不得把验证后的文件改名为 `validated-selection.json`。只有 validator 退出码为 0 时，
后续 finalize 才读取本次验证输出；失败时立即停止，不得沿用旧文件。

```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; node "$FOLLOW_UP_SKILL_DIR/scripts/validate-digest-selection.js" --request <absolute-request-path> --selection "$HOME/.follow-builders/state/selections/<digest-id>.json" --output "$HOME/.follow-builders/state/selections/validated/<digest-id>.json"
```

### Step 5: Finalize

```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; node "$FOLLOW_UP_SKILL_DIR/scripts/finalize-digest.js" --request <absolute-request-path> --selection "$HOME/.follow-builders/state/selections/validated/<digest-id>.json" --output-dir <absolute-output-directory>
```
...
```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; node "$FOLLOW_UP_SKILL_DIR/scripts/deliver.js" --active <absolute-output-directory>/active.json --destination <stdout|telegram|email> [--confirm-destination] --result-out <absolute-delivery-result-path>
```

自动运行必须显式标记 scheduled，使 deliver 再次执行 TOCTOU 门禁：

```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; node "$FOLLOW_UP_SKILL_DIR/scripts/deliver.js" --active <absolute-output-directory>/active.json --destination <stdout|telegram|email> --scheduled --result-out <absolute-delivery-result-path>
```

默认 stdout 也必须运行：

```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; node "$FOLLOW_UP_SKILL_DIR/scripts/deliver.js" --active <absolute-output-directory>/active.json --destination stdout --result-out <absolute-delivery-result-path>
```
...
```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; node "$FOLLOW_UP_SKILL_DIR/scripts/resolve-delivery.js" <attempt-id> delivered --result-out <absolute-resolution-result-path>
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; node "$FOLLOW_UP_SKILL_DIR/scripts/resolve-delivery.js" <attempt-id> suppress --result-out <absolute-resolution-result-path>
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; node "$FOLLOW_UP_SKILL_DIR/scripts/resolve-delivery.js" <attempt-id> retry --confirm-external-retry [--destination stdout|telegram|email] --result-out <absolute-resolution-result-path>
```
...
```bash
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; node "$FOLLOW_UP_SKILL_DIR/scripts/deliver.js" --active <absolute-output-directory>/active.json --destination <stdout|telegram|email> --resume-attempt <replacement-attempt-id> [--confirm-destination] --result-out <absolute-delivery-result-path>
```
...
FOLLOW_UP_SKILL_DIR="${FOLLOW_UP_SKILL_DIR:-$HOME/.follow-builders/releases/0.4.0-beta.4}"; cp "$FOLLOW_UP_SKILL_DIR/prompts/<filename>.md" ~/.follow-builders/prompts/<filename>.md
```
