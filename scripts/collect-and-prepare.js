...
import { chmod, lstat, readFile, mkdir, rm } from 'node:fs/promises';
...
export async function collectAndPrepare({
  ...
  loadIntent = loadCheckpointIntent,
  publishRun = publishBatchRun,
  publishPointers = publishLatestPointers,
  commitCheckpoints = invokeCheckpointCommit,
  prepare,
  validateBatches,
  onUnsafeBatches,
} = {}) {
...
export async function main({ argv = process.argv.slice(2), stdout = process.stdout, stderr = process.stderr,
  ...
  loadIntent = loadCheckpointIntent, commitCheckpoints = invokeCheckpointCommit,
  preparation = {},
} = {}) {
  try {
    ...
    const acquisitionInfo = await lstat(acquisitionDir);
    if (!acquisitionInfo.isDirectory() || acquisitionInfo.isSymbolicLink()) throw new Error('unsafe-acquisition-directory');
    await chmod(acquisitionDir, 0o700);
    ...
    try {
      ...
      const result = await collectAndPrepare({ config, now, userDir, invokeRun, loadIntent, commitCheckpoints,
