...
更新日期：2026-09-09。基线：`feature/v0.3.0-acquisition-runtime` / `dcfb979`。

本文补充现有采集母计划，不改变 v0.4–v0.7 的来源分配。版本顺序是开发建议，尚无人员和
工期承诺；以下均为待完成工作，不代表已发布能力。当前事实见 [项目进度](project-progress.md)。

## v0.3.0：先完成可安装、可运行、可验证的闭环

依赖顺序：R1 → R2 → R3 → R4。R5 可与收尾开发并行积累，但不能跳过观察天数。
